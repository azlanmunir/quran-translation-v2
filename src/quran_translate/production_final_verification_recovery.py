"""Non-destructive recovery for exhausted final-verification contracts."""
...
from .production_verification_recovery import _recovery_instruction


RECOVERY_VERSION = "final-verification-contract-recovery-v1"
RECOVERY_ATTEMPTS = (3, 4)
RECOVERY_MANIFEST = "FINAL_VERIFICATION_RECOVERY_MANIFEST.json"
RECOVERY_COMPLETE = "FINAL_VERIFICATION_RECOVERY_COMPLETE.json"
...
def recover_final_verification_units(
    *,
    base: Path,
    ...
    """Make fresh, checkpointed requests only for missing final-verification units."""
    ...
    for attempt in RECOVERY_ATTEMPTS:
        for unit in units:
            ...
            input_hash = _stage_input_hash("final_verification", unit, hash_payload)
            ...
            if load_artifact(
                artifact_path(base, unit, "final_verification"), input_hash, validator
            ):
                continue
            ...
            job_path = (
                base
                / "jobs"
                / f"final-verification-sync-recovery-a{attempt}-{unit.unit_id}.json"
            )
            row: dict[str, Any]
            submitted = False
            if job_path.exists():
                ...
                if job.get("request_hash") != request_hash:
                    raise ProductionError(
                        f"Final-verification recovery job mismatch: {job_path}"
                    )
                row = job.get("row")
                ...
                try:
                    ...
                    atomic_json(
                        unit_dir(base, unit)
                        / f"final-verification-recovery-attempt{attempt}-FAILED.json",
                        {
            ...
            atomic_json(
                artifact_path(base, unit, "final_verification"),
                {
                    "input_hash": input_hash,
                    "model": GEMINI_MODEL,
                    "transport": "sync-recovery",
    ...
    pending = [
        unit.unit_id
        for unit in units
        if not artifact_path(base, unit, "final_verification").exists()
    ]
    result = {
        "version": RECOVERY_VERSION,
    ...
    if pending:
        raise ProductionError(
            "Final-verification recovery exhausted fresh attempts for: "
            + ", ".join(pending)
        )
    return result
...
def _target_units(base: Path, units: list[ProductionUnit]) -> list[ProductionUnit]:
    ...
    if manifest_path.exists():
        ...
        if not isinstance(target_ids, list) or any(
            ...
            raise ProductionError("Final-verification recovery manifest has bad targets")
        return [by_id[str(unit_id)] for unit_id in target_ids]
    targets = [
        unit
        for unit in units
        if artifact_path(base, unit, "repair").exists()
        and not artifact_path(base, unit, "final_verification").exists()
        and (
            unit_dir(base, unit) / "final_verification-attempt2-FAILED.json"
        ).exists()
    ]
    if not targets:
        raise ProductionError("No exhausted final-verification units need recovery")
    return targets
...
def _freeze_recovery(base: Path, units: list[ProductionUnit]) -> None:
    ...
    record = {
        ...
        "recovery_instruction_code_sha256": file_hash(
            Path(__file__).with_name("production_verification_recovery.py")
        ),
        "model": GEMINI_MODEL,
        "max_output_tokens": GEMINI_MAX_TOKENS,
        ...
        "target_inputs": {
            unit.unit_id: {
                ...
                "verification_sha256": file_hash(
                    artifact_path(base, unit, "verification")
                ),
                "repair_sha256": file_hash(artifact_path(base, unit, "repair")),
    ...
    if path.exists():
        if json.loads(path.read_text(encoding="utf-8")) != record:
            raise ProductionError("Final-verification recovery manifest changed")
    else:
...
def main() -> None:
        ...
        **result,
        ...
        "artifacts": {
            unit.unit_id: file_hash(
                artifact_path(prepared_base, unit, "final_verification")
            )
            for unit in targets
        },
    }
    ...
    if complete_path.exists():
        ...
        if (
            ...
            raise ProductionError("Final-verification completion record changed")
        complete = current
    else:
