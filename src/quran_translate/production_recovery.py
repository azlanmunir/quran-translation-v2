"""Non-destructive recovery for exhausted Gemini critic contracts.

The frozen production runner deliberately reuses provider checkpoints. That is the
right default, but it means a contract-invalid response remains contract-invalid on
every resume. This command creates separately named recovery attempts, preserves all
earlier jobs and failure records, and writes only the canonical ``critic.json``
artifact after the original validator accepts the response.
"""
...
RECOVERY_VERSION = "critic-contract-recovery-v1"
RECOVERY_ATTEMPTS = (3, 4)
RECOVERY_MANIFEST = "CRITIC_RECOVERY_MANIFEST.json"
RECOVERY_COMPLETE = "CRITIC_RECOVERY_COMPLETE.json"
...
def _recovery_instruction(
    ...
    return (
        f"\n\n=== CONTRACT RECOVERY ATTEMPT {attempt} ===\n"
        "Earlier responses failed the interface contract, not necessarily the "
...
def recover_critic_units(
    *,
    ...
    """Issue fresh, checkpointed requests only for missing critic artifacts."""
    attempted: list[dict[str, Any]] = []
    for attempt in RECOVERY_ATTEMPTS:
        for unit in units:
            reader = _draft_reader(base, unit)
            original_user = critic_assignment(shared_by_unit[unit.unit_id], reader)
            hash_payload = {
                "system": critic_system,
                "reader": reader,
                "shared": shared_by_unit[unit.unit_id],
            }
            input_hash = _stage_input_hash("critic", unit, hash_payload)
            ...
            if load_artifact(artifact_path(base, unit, "critic"), input_hash, validator):
                continue
            ...
            job_path = (
                base
                / "jobs"
                / f"critic-sync-recovery-a{attempt}-{unit.unit_id}.json"
            )
            row: dict[str, Any]
            submitted = False
            if job_path.exists():
                ...
                if job.get("request_hash") != request_hash:
                    raise ProductionError(f"Recovery provider job mismatch: {job_path}")
                row = job.get("row")
                ...
                try:
                    ...
                    atomic_json(
                        unit_dir(base, unit)
                        / f"critic-recovery-attempt{attempt}-FAILED.json",
                        {
            ...
            atomic_json(
                artifact_path(base, unit, "critic"),
                {
                    "input_hash": input_hash,
                    "model": GEMINI_MODEL,
                    "transport": "sync-recovery",
    ...
    pending = [
        unit.unit_id
        for unit in units
        if not artifact_path(base, unit, "critic").exists()
    ]
    result = {
        "version": RECOVERY_VERSION,
    ...
    if pending:
        raise ProductionError(
            "Critic recovery exhausted fresh attempts for: " + ", ".join(pending)
        )
    return result
...
def _target_units(base: Path, units: list[ProductionUnit]) -> list[ProductionUnit]:
    ...
    if manifest_path.exists():
        ...
        if not isinstance(target_ids, list) or any(
            ...
            raise ProductionError("Recovery manifest has invalid target units")
        return [by_id[str(unit_id)] for unit_id in target_ids]
    targets = [
        unit
        for unit in units
        if not artifact_path(base, unit, "critic").exists()
        and (unit_dir(base, unit) / "critic-attempt2-FAILED.json").exists()
    ]
    if not targets:
        raise ProductionError("No exhausted critic units require recovery")
    return targets
...
def _freeze_recovery(base: Path, units: list[ProductionUnit]) -> None:
        ...
        "target_inputs": {
            unit.unit_id: {
                ...
                "attempt2_failure_sha256": file_hash(
                    unit_dir(base, unit) / "critic-attempt2-FAILED.json"
                ),
            }
            for unit in units
        },
    ...
    if path.exists():
        if json.loads(path.read_text(encoding="utf-8")) != record:
            raise ProductionError("Critic recovery manifest changed")
    else:
...
def main() -> None:
    ...
    complete = {
        **result,
        ...
        "artifacts": {
            unit.unit_id: file_hash(artifact_path(prepared_base, unit, "critic"))
            for unit in targets
        },
    }
    ...
    if complete_path.exists():
        ...
        if (
            ...
            or current_artifacts != complete["artifacts"]
        ):
            raise ProductionError("Critic recovery completion record changed")
        complete = current
    else:
