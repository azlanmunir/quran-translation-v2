...
from quran_translate.production_verification_recovery import (
    recover_verification_units,
)
...
class ProductionVerificationRecoveryTests(unittest.TestCase):
    def test_recovery_uses_fresh_jobs_and_preserves_prior_records(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            atomic_json(
                base / "units" / unit.unit_id / "critic.json",
                {
                    "result": [
                        {
                            "ayah": 1,
                            "findings": [
                                {
                                    ...
                                    "where": "The opening.",
                                    "arabic_ground": "النص",
                                    "explanation": "Use the revised wording.",
                                }
                            ],
                            "verdict": "revise",
                        }
            ...
            atomic_json(
                base / "units" / unit.unit_id / "revision.json",
                {
                    "result": {
                        "ayahs": [{"ayah": 1, "english": "The beginning."}],
                        "decisions": [
                            {
                                "finding_id": "f-1-0",
                                "decision": "applied",
                                "reason": "Corrected for the test.",
                            }
                        ],
                    }
                },
            ...
            prior_failure = (
                ...
                / "units"
                / unit.unit_id
                / "verification-attempt2-FAILED.json"
            )
            ...
            self.assertTrue(
                (
                    base
                    / "jobs"
                    / "verification-sync-recovery-a3-s001_001_001.json"
                ).is_file()
            )
            self.assertTrue(
                (
                    ...
                    / "units"
                    / unit.unit_id
                    / "verification-recovery-attempt3-FAILED.json"
                ).is_file()
            )
            artifact = json.loads(
                (
                    base / "units" / unit.unit_id / "verification.json"
                ).read_text()
            )
            ...
            self.assertIn("post-revision verification", client.calls[0]["user"])

            no_call_client = SequenceGeminiClient([])
            resumed = recover_verification_units(
                base=base,
                units=[unit],
                verses={(1, 1): "النص"},
                shared_by_unit={unit.unit_id: "Shared evidence."},
